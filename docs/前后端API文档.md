...
> **说明**:认证接口(3.1~3.8)由后端直接返回 `Result`,HTTP 状态码固定为 200(框架级校验异常除外),业务结果以响应体中的 `code` 为准。
...
**接口**: `POST /auth/deregister`

**描述**: 永久删除当前账号及其所有数据，操作不可逆。需要用户再次输入登录密码进行二次确认。
...
```json
{
  "user_password": "Password123"    // 必填，当前账号的登录密码，用于二次确认
}
```
...
- 400: 密码错误
- 401: Token无效或已过期
- 403: 权限不足（非 ROLE_USER）

**说明**:
- 账号注销后，数据库中的用户记录及所有关联数据将被**永久删除**，无法恢复
- 注销成功后当前 Token 立即失效
...
**描述**: 修改账号密码。根据是否携带有效 Token 分为两种场景：
- **已登录**：从 JWT Token 中自动解析用户身份，无需传入用户名
- **未登录**：需在请求体中传入 `user_email` 字段

**请求头（已登录时必填）**: `Authorization: Bearer {token}`

**请求参数**:
```json
{
  "user_email": "user@example.com",    // 未登录时必填；已登录时可省略（从Token中解析）
  "old_password": "OldPassword123",    // 必填，当前密码
  "new_password": "NewPassword456"     // 必填，新密码，至少8位，包含字母和数字
}
```
...
**错误响应**:
- 400: 原密码错误 / 新密码格式不合规 / 未登录时 `user_email` 缺失或用户不存在

**说明**:
- 已登录状态下，即使请求体中传入了 `user_email`，也以 Token 中解析的身份为准，传入的邮箱字段会被忽略
- 密码修改成功后，当前 Token 立即失效，用户需要重新登录
