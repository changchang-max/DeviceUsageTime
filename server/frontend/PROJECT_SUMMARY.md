...
### 5. 对接真实后端
- 开发模式通过 Vite 代理(/api、/ws)对接后端 Spring Boot(8080)
- 支持 Token 认证与秘钥认证两种访问方式
- 秘钥可能含特殊字符,已统一走 URL 编码传输
...
### 对接说明
- 前端已不包含任何 Mock 服务,所有接口直接对接真实后端
- 数据展示依赖后端数据(客户端上传),无数据时页面显示空状态
...
前端项目已完整构建,包含所有必需的功能模块。所有组件、页面、状态管理、路由、API接口均已实现,所有接口对接真实后端(Mock 已移除)。项目采用现代化的技术栈,代码结构清晰,类型安全,易于维护和扩展。

开发者需先启动后端 Spring Boot 服务,前端通过 Vite 代理(/api 与 /ws)访问真实接口。
